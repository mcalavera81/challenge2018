...
import static demo.support.parser.JsonParser.RestResponseField.PAYLOAD;
import static demo.support.parser.JsonParser.RestResponseField.SUCCESS;


public class TestOrderHelpers {
    ...
    private static DiffOrder diffOrderCancelled(String id, OrderType type) {
        ...
        return new DiffOrder(type,data, DateTimeHelpers.now(),randStatus());
    }
    ...
    public static Bid randBid(){
        return new Bid(randOrderData(), DateTimeHelpers.now());
    }

    public static Ask randAsk(){
        return new Ask(randOrderData(), DateTimeHelpers.now());
    }
